...
<div class="dashboard-cards">
    <div class="card">
        ...
        <div class="card-content">
            <h3>Total Guru</h3>
            <p class="card-number">{{ \App\Models\User::where('hak_akses', 'guru')->count() }}</p>
    ...
    <div class="card">
        ...
        <div class="card-content">
            <h3>Total Siswa</h3>
            <p class="card-number">{{ \App\Models\Siswa::count() }}</p>
    ...
    <div class="card">
        ...
        <div class="card-content">
            <h3>Total Kelas</h3>
            <p class="card-number">{{ \App\Models\Kelas::count() }}</p>
            <small class="card-change">Kelas aktif</small>
        </div>
    </div>
</div>

<!-- CHART SISWA — row 1 (2 kolom sama besar) -->
...
<!-- CHART GENDER + STATUS — row 2 (2 kolom sama besar) -->
<div class="charts-section">
...
<!-- CHART GURU — row 3 (2 kolom sama besar) -->
<div class="charts-section">
    <div class="chart-container-large">
        <div class="chart-card">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @endphp
                        <span class="badge badge-{{ $badgeClass }}">{{ $siswa->status_siswa }}</span>
                    </td>
                </tr>
                @endforeach
            </tbody>
...
<script>
    window.chartPerkembanganSiswaData = {
        ...
        datasets: [{ label: 'Jumlah Siswa', data: @json($dataSiswa), fill: true, tension: 0.4 }]
    };
    window.chartDistribusiKelasData = {
        labels: @json($labelKelas),
        datasets: [{ data: @json($dataKelas) }]
    };
    window.chartGenderData = {
        labels: ['Laki-laki', 'Perempuan'],
        datasets: [{ data: @json($gender) }]
    };
    window.chartStatusSiswaData = {
        labels: ['Aktif', 'Lulus', 'Pindah', 'Keluar'],
        datasets: [{ data: @json($status) }]
    };
    window.chartPerkembanganGuruData = {
        labels: @json($labelTahunGuru),
        datasets: [{ label: 'Jumlah Guru', data: @json($dataGuru), fill: true, tension: 0.4 }]
    };
    window.chartStatusGuruData = {
        labels: ['Aktif', 'Nonaktif'],
        datasets: [{ data: @json($statusGuru) }]
    };
</script>
